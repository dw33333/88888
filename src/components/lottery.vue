            ...
            <div class="lottery-header w">
                <div class="lottery-header-top w">
                    <div class="lottery-nav">
                        <ul class="clearfix">
                            <li  v-for="(v,index) in menus" @click="change(v.name,index)" :class="fc_type==v.name?'active':''" v-show="index<=10">
                                <router-link :to="{name:'lottery',params:{id:v.name}}">
                                    {{v.short_name}}
                                </router-link>
                            </li>
                            <li @mouseover="isMenu=true" @mouseout="isMenu=false"   v-for="(v,index) in menus"  v-show="index==10" :class="more!='更多玩法'?'active':''">
                                <a href="javascript:;" class="moreNav ">
                                    {{more}}
                                    <span class="caret"></span>
                                </a>
                                <ul v-show="isMenu" class="dropdown-menu" @mouseover="isMenu=true" @mouseout="isMenu=false" >
                                    <li  v-for="(v,index) in menus" @click="change(v.name,index,v.short_name);" :class="fc_type==v.name&&index>10?'active':''" v-show="index>10">
                                        <router-link :to="{name:'lottery',params:{id:v.name}}">
                                            {{v.short_name}}
                                        </router-link>
                                    </li>
                ...
                <div class="bet-box-content">
                    ...
                    <div class="pk10 clearfix" v-if="currT==204">
                        <div class="table-bet-top "  v-for="(v,index) in mainData " :class="currN=='单号1-10'?'fl':''">
                            <p >{{v[0].playd_type_name}}</p>
                            <div class="ul-width"  v-if="v[0].playd_type_name=='冠亚和'">
                                <ul >
                                    <li>
                                        <span >选项</span>
                                        <span >赔率</span>
                            ...
                            <ul class="select">
                                <li v-for="(vv,i) in v" :data-id="vv.play_id" v-if="i<=16 && vv.odds!=null" @click="isAct($event);">
                                    ...
                                    <span class="ff9b39" >{{vv.odds}}</span>
                                    <span> <input type="number" name="" class="input_money" v-on:input="parentAct($event)" :data-min="vv.min_charge" :data-max="vv.max_charge" :data-id="vv.play_id" :data-title="vv.playd_type_name" :data-playd_info="vv.playd_info" :data-name="vv.name"  :data-odds="vv.odds" onkeyup="value=value.replace(/[^\d]/g,'') " maxlength="5"></span>
                                </li>
                            </ul>
                            <div class="ul-width"  v-for="(vv,i) in v" v-if="i==16">
                                <ul >
                                    <li>
                                        <span >选项</span>
                                        <span >赔率</span>
                            ...
                            <ul class="clearfix select">
                                <li v-for="(vv,i) in v" :data-id="vv.play_id" v-if="i>16" @click="isAct($event);">
                                    ...
                                    <span class="ff9b39" >{{vv.odds}}</span>
                                    <span> <input type="text" name="" class="input_money" :data-min="vv.min_charge" :data-max="vv.max_charge" :data-id="vv.play_id" :data-title="vv.playd_type_name" :data-playd_info="vv.playd_info"  :data-name="vv.name" :data-odds="vv.odds" onkeyup="value=value.replace(/[^\d]/g,'') " maxlength="5"></span>
                                </li>
                            </ul>
                        </div>
...
//                this.editMount = this.editMount-0;
                for(let i =0;i<nod.length;i++){
                    ...
                    let obj = {};
                    if(nod[i].value&&nod[i].value!=0 || !this.ifCofm || this.currBall=='正肖' ||this.currBall=='特码生肖'){
                        ...
                        if(this.ifCofm==false){
                            ...
                            if(this.currBall=='正肖' || this.currBall=='特码生肖')this.moneyBox += this.editMount
                            else this.moneyBox += (nod[i].value-0);
                        }
                        arr.push(obj);
                    }
                }
                if(arr.length&&parent.length ){
                    this.arrTicket =[];
                    this.arrTicket = arr;
